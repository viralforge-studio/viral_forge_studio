...
export function IdeaJsonPasteDialog({
  projectId,
  onUploaded,
}: {
  projectId: string;
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!response.ok) {
        setError(result.error ?? "Unable to save pasted idea JSON.");
        setValidationErrors(result.validationErrors ?? []);
        return;
      }

      setMessage("Idea JSON pasted and validated successfully.");
      onUploaded();
    } catch (pasteError) {
      if (pasteError instanceof SyntaxError) {
        setError("Invalid pasted JSON.");
      } else {
        setError("Unable to save pasted idea JSON.");
      }
    } finally {
      setIsSaving(false);
    }
  ...
  return (
    <div className="grid gap-2">
      ...
      {isOpen ? (
        <Card>
          <CardHeader>
            <CardTitle>Paste IdeaGeneration JSON</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4">
            <Textarea
              ...
              placeholder="Paste a full IdeaGeneration JSON object here."
            />
