...
export function KlingPromptsPromptEditor({
  value,
  onChange,
  onSave,
  onReset,
    ...
    <div className="grid gap-4">
      <StickyActionBar>
        ...
        <Button variant="secondary" onClick={onGenerate} disabled={isGenerating}>
          ...
          {isGenerating ? "Generating..." : "Generate Kling Prompts"}
        </Button>
      </StickyActionBar>
      <Textarea value={value} onChange={(event) => onChange(event.target.value)} className="min-h-[30rem] font-mono text-xs leading-6" />
    </div>
  );
}
